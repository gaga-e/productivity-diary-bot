...
async def cmd_job(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    status_msg = await update.message.reply_text(
        f'🔍 Searching LinkedIn, Indeed, RemoteOK, Adzuna, and Jooble for "{keywords}" '
        f'({location}, last {cfg.JOB_LOOKBACK_HOURS}h)...'
    )
...
async def _run_search_and_respond(update, context, keywords, location, status_msg):
    ...
    if cached:
        ...
        try:
            ...
            await status_msg.edit_text(f"⚠️ Search failed unexpectedly: {e}")
            return

    if not jobs:
        await status_msg.edit_text(
            f'😕 No jobs found for "{keywords}" in the last {cfg.JOB_LOOKBACK_HOURS}h.\n\n{_status_summary(status)}'
        )
        return
    ...
    header = f'Found {len(jobs)} jobs for "{keywords}" ({location})\n{_status_summary(status)}'
    await status_msg.edit_text(f"✅ {header.splitlines()[0]}")
...
async def btn_job_page(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        text=text, parse_mode="Markdown", reply_markup=keyboard, disable_web_page_preview=True
    )
